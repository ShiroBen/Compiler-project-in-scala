...
object NameAnalyzer extends Pipeline[N.Program, (S.Program, SymbolTable)] {
  def run(ctx: Context)(p: N.Program): (S.Program, SymbolTable) = {
    ...
    } // checks each module has unique name

    modNames.keys.toList foreach table.addModule // module add string into table, not identifier

    // Step 2: Check name uniqueness in modules, i.e. things defined in modules has unique name
    p.modules.foreach { m =>
