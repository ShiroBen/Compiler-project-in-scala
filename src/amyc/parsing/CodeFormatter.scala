...
// The lexer for Amy.
object CodeFormatter extends Pipeline[List[File], String] with Lexers{
  ...
  def formatter(sb: StringBuilder,tkList: Iterator[Token]): StringBuilder = {
    var idt: Int = 0
    for (token <- tkList) {
      // instead of recognizing tokens that can start a line, we recognize tokens that can end a line
      token match {
        case KeywordToken(value) => sb.append(value + " ")
        case PrimTypeToken(value) => sb.append(value + " ")
        case BoolLitToken(value) => sb.append(value.toString + " ")
        case OperatorToken(value) => sb.append(value + " ")
        case IdentifierToken(value) => sb.append(value + " ")
        case IntLitToken(value) => sb.append(value.toString + " ")
        case StringLitToken(value) => sb.append("\"" + value + "\" ")
        case DelimiterToken(value) => 
          value match {
            case ";" => sb.append(value + "\n" + " " * idt)
            case "}" => idt -= 1; sb.append("\n" + " " * idt + value + "\n" + " " * idt)
            case "{" => sb.append("\n" + " " * idt + value); idt += 1; sb.append("\n" + " " * idt) // definition body will start from a new line
            case _ => sb.append(value + " ")
          }
        case CommentToken(value) => sb.append("// " + value + "\n" + " " * idt)
        case _ => sb
      }
    }
    sb
  ...
  override def run(ctx: amyc.utils.Context)(files: List[File]): String = {
    var it = Seq[Token]().iterator

    for (file <- files) {
      val source = Source.fromFile(file.toString, SourcePositioner(file))
      it ++= lexer.spawn(source)
    }
    val sb = new StringBuilder()
    val formattedCode = formatter(sb,it).mkString
    formattedCode
  }
}

/** Extracts all tokens from input and displays them */
object DisplayCodes extends Pipeline[String, Unit] {
  override def run(ctx: Context)(fmtCode: String): Unit = {
    println(fmtCode)
  }
}